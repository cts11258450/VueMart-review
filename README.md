...
專案包含商品列表、商品詳情、購物車、會員登入、結帳流程、訂單建立、會員訂單紀錄、訂單詳情，以及後台商品管理 CRUD 等功能。

這個專案主要用來練習 Vue 3 Composition API、元件拆分、Pinia 狀態管理、Vue Router 路由管理、表單驗證、localStorage 資料保存、RESTful API 串接，以及完整的前端購物與後台管理流程。
...
專案使用 Pinia action 搭配 async / await 管理資料取得、訂單建立與後台商品 CRUD 流程，同時處理 loading、error、empty state、not found、submitting、creating、updating、deleting 與避免重複請求等情境，讓專案架構更接近真實前端開發流程。
...
* 使用 `productApi.js` 與 `orderApi.js` 封裝 API 請求
* 使用 `response.ok` 與 `response.json()` 處理 API 回應
* 使用 async / await 與 try / catch / finally 處理資料載入、訂單建立與商品 CRUD 流程
...
此權限僅為前端模擬，真實專案中管理員權限應由後端登入 API 回傳，並由後端 API 驗證新增、編輯與刪除商品的操作權限。
...
前端會透過 `fetch()` 向 JSON Server 取得商品資料，透過 POST API 建立訂單，並透過 POST / PATCH / DELETE API 操作後台商品 CRUD。

若未啟動 JSON Server，商品頁會顯示錯誤狀態，訂單送出會顯示錯誤 Toast，後台新增、編輯與刪除商品也會顯示錯誤 Toast，提示確認 JSON Server 是否已啟動。
...
本專案使用 JSON Server 模擬後端 REST API，商品資料與訂單資料存放於 `db.json`。

開發環境中，前端會透過 `src/api/productApi.js` 與 `src/api/orderApi.js` 串接 JSON Server。
...
線上 Demo 也能穩定顯示商品、完成結帳流程，並模擬後台商品 CRUD 操作
```

由於 GitHub Pages 無法執行 JSON Server，線上 Demo 中的新增、編輯與刪除商品屬於前端 fallback 模擬操作；重新整理頁面後，商品資料會回到原本的 mockProducts 狀態。
...
目前已完成主要電商流程，並將商品資料、訂單建立與後台商品管理改為 API 架構：
...
目前商品資料、訂單建立與後台商品 CRUD 在開發環境中使用 JSON Server 模擬後端 REST API。

正式部署環境則使用 mock data 與 fallback 機制，確保 GitHub Pages 線上 Demo 可以正常瀏覽商品、建立訂單，並模擬後台商品新增、編輯與刪除流程。
...
正式部署環境中，商品資料使用 `mockProducts` 作為 fallback，訂單建立與後台商品 CRUD 則使用前端 fallback 模擬操作成功，確保 GitHub Pages 線上 Demo 可以完整操作。

會員登入、購物車與訂單資料目前使用 localStorage 模擬前端資料保存。

目前的後台管理員權限屬於前端 Route Guard 模擬，真實專案中仍需由後端 API 驗證使用者是否具備新增、編輯與刪除商品的權限。

若未來擴充，可將 JSON Server 替換為真實後端 API，並加入資料庫、真實會員系統、金流串接、圖片上傳與後台訂單管理功能。